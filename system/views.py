...
from utils.surv_code import SurvCode
...
class ProjectInfoViewSet(GroupbyMixin, MyModelViewSet, metaclass=SimpleViewSetBase):
    ...
    def activate(self, request):
        if self.model.activated():
            raise CodeMsgException(SurvCode.ALREADY_ACTIVED, '项目不能重复激活')

        data = request.data
        project_name = data.get('project_name')
        if not project_name:
            raise CodeMsgException(SurvCode.INVALID_PROJECT_NAME, '项目名称无效')
        auth_code = data.get('auth_code')
        if not self.model.check_auth_code(auth_code):
            raise CodeMsgException(SurvCode.INVALID_AUTH_CODE, '授权码无效或已使用')

        username = data.get('username')
        password = data.get('password')
        if not username:
            raise CodeMsgException(SurvCode.INVALID_USERNAME, '用户名不能为空')
        if User.objects.filter(username=username).count() > 0:
            raise CodeMsgException(SurvCode.INVALID_USERNAME, '用户名已经存在')
        if not password:
            raise CodeMsgException(SurvCode.INVALID_PASSWORD, '密码不能为空')
